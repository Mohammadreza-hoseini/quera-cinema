...
class Comment:
    ...
    def add_comment_to_movie(user_id: str) -> str:
        ...
        while result is None:
            ...
            result = cursor.fetchone()
        movie_name = result[0]
        cursor.execute(f"INSERT INTO Comment(id, user_id, created_at, body, movie_name)"
    ...
    def reply_to_comment(user_id: str, comment_id: str) -> None:
        ...
        cursor.execute(f"SELECT movie_name FROM Comment WHERE id='{comment_id}' and user_id='{user_id}'")
        result = cursor.fetchone()
        print(result)
        if result is not None:
            body = input('write your reply text: ')
            movie_name = result[0]
            cursor.execute(f"INSERT INTO Comment(id, user_id, created_at, body, p_comment_id, movie_name)"
